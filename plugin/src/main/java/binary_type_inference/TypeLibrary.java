...
public class TypeLibrary {
  ...
  private TypeLibrary(
      CTypeMapping mapping, Map<String, DataType> type_constants, DataType unknownType, DataTypeManager dtm) {
    this.mapping = mapping;
  ...
  private ParameterDefinition BuildParamDefForParam(DataType curr_type, int node_index, int param_idx, int param_node) {
  ...
  private DataType build_function(int node_index, Function func) {
    ...
    var max_ind = func.getParametersList().stream()
        ...
        .mapToObj(
            (int param_idx) -> {
              if (param_map.containsKey(param_idx)) {
                return this.BuildParamDefForParam(res_type,
                    node_index, param_idx, param_map.get(param_idx));
              } else {
  ...
  public Types buildMapping() {
    ...
    var tid_mapping_memo = this.mapping.getTypeVariableReprNodesList().stream()
        .map(
  ...
  public static TypeLibrary parseFromInputStream(
      InputStream target, Map<String, DataType> type_constants, DataType unknownType, DataTypeManager dtm)
      throws IOException {
