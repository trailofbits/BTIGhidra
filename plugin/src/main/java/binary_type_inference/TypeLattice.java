...
import constraints.Constraints.DerivedTypeVariableOrBuilder;
...
public class TypeLattice {
    ...
    private static Retval constraintsForReturn(Tid tid, DataType ty) {
        ...
        var dtv = DerivedTypeVariable.newBuilder().setBaseVar(func_tvar)
                .addFieldLabels(FieldLabel.newBuilder().setOutParam(0)).build();

        var repr_for_ty = TypeLattice.representation_for_datatype(ty);

        repr_for_ty.second.addSubtyCons(SubtypingConstraint.newBuilder().setLhs(dtv).setRhs(repr_for_ty.first).build());
    ...
    private static Retval constraintsForParam(int idx, Tid tid, DataType ty) {
        ...
        var new_cons = SubtypingConstraint.newBuilder().setLhs(repr.first).setRhs(dtv_for_param_of_tid(idx, tid))
                .build();
    ...
    private static DerivedTypeVariable dtv_for_param_of_tid(int idx, Tid tid) {
        return DerivedTypeVariable.newBuilder().setBaseVar(tid_to_tvar(tid))
                .addFieldLabels(FieldLabel.newBuilder().setInParam(idx)).build();
    }
    ...
    private static DerivedTypeVariable.Builder get_ptr_dtv_for_type(Pointer pty,
            constraints.Constraints.Pointer load_or_store) {
    ...
    private static Pair<DerivedTypeVariable, Retval> representation_for_pointer(Pointer ptr) {
        var load_repr = TypeLattice.get_ptr_dtv_for_type(ptr, constraints.Constraints.Pointer.POINTER_LOAD_UNSPECIFIED);
        ...
        return new Pair<DerivedTypeVariable, Retval>(data_type_to_derived_variable(ptr).build(), retval);
    }
    ...
    private static Pair<DerivedTypeVariable, Retval> representation_for_structure(Structure struct) {
        ...
        for (var comp : struct.getComponents()) {
            ...
            var field_access = struct_var.addFieldLabels(FieldLabel.newBuilder()
                    .setField(Field.newBuilder().setBitSize(comp.getLength() * 8).setByteOffset(comp.getOffset())))
                    .build();

            tot.addSubtyCons(SubtypingConstraint.newBuilder().setLhs(repr_of_field.first).setRhs(field_access).build());
        }

        return new Pair<DerivedTypeVariable, Retval>(TypeLattice.data_type_to_derived_variable(struct).build(), tot);
    }
    ...
    private Retval collectSignatureConstraints() {
        return this.fixed_signatures.entrySet().stream()
                .map((var sig) -> TypeLattice.constraintsForSignature(sig.getKey(), sig.getValue())).reduce(
                        new Retval(),
                        (Retval x, Retval y) -> x.merge(y));
    }
    ...
    private Stream<Pair<String, String>> applyLessThanStrategies(DataType type_const) {
        ...
        for (var strat : this.less_than_relation_strategy) {
            ...
            if (maybe_res.isPresent()) {
                ...
                return dts.stream().map((DataType things_greater) -> new Pair<String, String>(const_str,
                        TypeLattice.data_type_to_type_variable(things_greater)));
            }
    ...
    private Stream<Pair<String, String>> constantsToLattice(Set<DataType> constants) {
        var bottom_cons = constants.stream().map((DataType target_constant) -> {
            return new Pair<String, String>(OutputBuilder.BOTTOM_STRING,
                    TypeLattice.data_type_to_type_variable(target_constant));
        });

        var top_cons = constants.stream().map((DataType target_constant) -> {
            return new Pair<String, String>(TypeLattice.data_type_to_type_variable(target_constant),
                    OutputBuilder.TOP_STRING);
        });

        var generated_cons = constants.stream().flatMap((DataType target_constant) -> {
